...
---
name: ai-search
description: 'Search AI workspace notes by tags, project, kind, date, or full text — across scratch, local, and saved tiers'
agent: copilot
...
${input:tier:Filter by tier: scratch | local | saved | all (default: all):all}

## Instructions

You are an AI workspace search assistant. Find notes matching the user's query and filters.

### Steps

1. Determine which tiers to search:
   - `${input:tier}` == "all" or blank → search scratch/, local/, saved/
...
saved    mcp-servers/2026-02/2026-02-21_sse.md       decision   mcp-servers    sse, transport
local    2026-02-20_java-generics.md                 note       java           java, generics
```

7. If no results: suggest broadening the search (remove a filter, try a different term)
8. If results found: offer to open a specific file or promote one to saved/

### Tip for the user

To run this from the terminal instead:
```powershell
.\ai\scripts\ai.ps1 search <query> --tag <tag> --project <project> --kind <kind> --date <YYYY-MM> --tier <tier>
```
```bash
./ai/scripts/ai.sh search <query> --tag <tag> --project <project> --kind <kind>
```
```
