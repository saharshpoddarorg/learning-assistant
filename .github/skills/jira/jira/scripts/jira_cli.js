...
async function main() {
  ...
  const env = loadEnv();
  if (!env.JIRA_PAT_TOKEN) {
    console.log(JSON.stringify({ success: false, error: 'Missing JIRA_PAT_TOKEN in .env file.' }));
    process.exit(1);
  }
  if (!env.JIRA_BASE_URL) {
    console.log(JSON.stringify({ success: false, error: 'Missing JIRA_BASE_URL in .env file.' }));
    process.exit(1);
  }

  const client = new RestClient(env.JIRA_BASE_URL, env.JIRA_PAT_TOKEN);
