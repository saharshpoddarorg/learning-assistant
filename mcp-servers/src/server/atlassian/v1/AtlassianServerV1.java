package server.atlassian.v1;

import server.atlassian.v1.config.AtlassianConfigLoader;
 ...
 *
 * @deprecated Use {@link server.atlassian.v2.AtlassianServerV2} instead.
 *     v2 provides 91 tools (vs 44), typed input schemas, and browser-based
 *     OAuth 2.0 login in addition to API token / PAT auth.
 *
...
public class AtlassianServerV1 implements McpServer {

    private static final Logger LOGGER = Logger.getLogger(AtlassianServerV1.class.getName());
     ...
     */
    public AtlassianServerV1(final AtlassianServerConfig serverConfig) {
    ...
    public String handleToolCall(final String toolName, final Map<String, String> args) {
        final var result = toolHandler.handleToolCall(toolName, args);
        return result.text();
    }
    ...
    /**
     * Factory method that creates a fully wired v1 server using the standard
     ...
     * this package.
     *
     * @return a ready-to-start v1 server instance
     */
    public static AtlassianServerV1 wireServer() {
        return new AtlassianServerV1(loadConfig());
    }
    ...
    public static void main(final String[] args) {
        ...
        final var server = new AtlassianServerV1(serverConfig);
    ...
    private static AtlassianServerConfig loadConfig() {
        try {
            ...
            return AtlassianServerConfig.cloudDefaults(
                    ...
                    new server.atlassian.v1.model.AtlassianCredentials(
                            "", "placeholder-token",
                            server.atlassian.v1.model.AuthType.PERSONAL_ACCESS_TOKEN));
        }
    }
     ...
     */
    private static void printToolList(final AtlassianServerV1 server) {
     ...
     * @param server the server instance
     */
    private static void runDemo(final AtlassianServerV1 server) {
